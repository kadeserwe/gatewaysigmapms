...
export class CritereComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.critereService
      ...
      .subscribe(
        (res: HttpResponse<ICritere[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ICritere): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInCriteres(): void {
    this.eventSubscriber = this.eventManager.subscribe('critereListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICritere[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
